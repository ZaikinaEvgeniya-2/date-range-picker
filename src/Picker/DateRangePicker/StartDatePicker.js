...
export const StartDatePicker = ({ value, onChange, endDate, maxHoursDiff }) => {
  ...
  const getAvailableStartDate = useCallback(
    (current) => {
      try {
        ...
        if (minDate && (current.isSame(minDate) || current.isBefore(minDate))) {
          return minDate;
        }

        if (endDate && (current.isSame(endDate) || current.isAfter(endDate))) {
